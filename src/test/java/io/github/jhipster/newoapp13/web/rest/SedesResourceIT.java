...
public class SedesResourceIT {
    ...
    @Test
    @Transactional
    public void getAllSedesByVideoIsEqualToSomething() throws Exception {
        // Initialize the database
        sedesRepository.saveAndFlush(sedes);

        // Get all the sedesList where video equals to DEFAULT_VIDEO
        defaultSedesShouldBeFound("video.equals=" + DEFAULT_VIDEO);

        // Get all the sedesList where video equals to UPDATED_VIDEO
        defaultSedesShouldNotBeFound("video.equals=" + UPDATED_VIDEO);
    }

    @Test
    @Transactional
    public void getAllSedesByVideoIsInShouldWork() throws Exception {
        // Initialize the database
        sedesRepository.saveAndFlush(sedes);

        // Get all the sedesList where video in DEFAULT_VIDEO or UPDATED_VIDEO
        defaultSedesShouldBeFound("video.in=" + DEFAULT_VIDEO + "," + UPDATED_VIDEO);

        // Get all the sedesList where video equals to UPDATED_VIDEO
        defaultSedesShouldNotBeFound("video.in=" + UPDATED_VIDEO);
    }

    @Test
    @Transactional
    public void getAllSedesByVideoIsNullOrNotNull() throws Exception {
        // Initialize the database
        sedesRepository.saveAndFlush(sedes);

        // Get all the sedesList where video is not null
        defaultSedesShouldBeFound("video.specified=true");

        // Get all the sedesList where video is null
        defaultSedesShouldNotBeFound("video.specified=false");
    }
