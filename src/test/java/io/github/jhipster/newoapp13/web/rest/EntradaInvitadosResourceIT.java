...
public class EntradaInvitadosResourceIT {
    ...
    private static final Instant DEFAULT_REGISTRO_FECHA = Instant.ofEpochMilli(0L);
    private static final Instant UPDATED_REGISTRO_FECHA = Instant.now().truncatedTo(ChronoUnit.MILLIS);
    private static final Instant SMALLER_REGISTRO_FECHA = Instant.ofEpochMilli(-1L);
    ...
    public void getAllEntradaInvitados() throws Exception {
        ...
        restEntradaInvitadosMockMvc.perform(get("/api/entrada-invitados?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].registroFecha").value(hasItem(DEFAULT_REGISTRO_FECHA.toString())))
    ...
    public void getEntradaInvitados() throws Exception {
        ...
        restEntradaInvitadosMockMvc.perform(get("/api/entrada-invitados/{id}", entradaInvitados.getId()))
            ...
            .andExpect(jsonPath("$.registroFecha").value(DEFAULT_REGISTRO_FECHA.toString()))
    ...
    @Test
    @Transactional
    public void getAllEntradaInvitadosByOficinaIsEqualToSomething() throws Exception {
        // Initialize the database
        entradaInvitadosRepository.saveAndFlush(entradaInvitados);
        Landing oficina = LandingResourceIT.createEntity(em);
        em.persist(oficina);
        em.flush();
        entradaInvitados.setOficina(oficina);
        entradaInvitadosRepository.saveAndFlush(entradaInvitados);
        Long oficinaId = oficina.getId();

        // Get all the entradaInvitadosList where oficina equals to oficinaId
        defaultEntradaInvitadosShouldBeFound("oficinaId.equals=" + oficinaId);

        // Get all the entradaInvitadosList where oficina equals to oficinaId + 1
        defaultEntradaInvitadosShouldNotBeFound("oficinaId.equals=" + (oficinaId + 1));
    }
