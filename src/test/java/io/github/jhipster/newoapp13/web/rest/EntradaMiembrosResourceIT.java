...
public class EntradaMiembrosResourceIT {
    ...
    @Test
    @Transactional
    public void getAllEntradaMiembrosByOficinaIsEqualToSomething() throws Exception {
        // Initialize the database
        entradaMiembrosRepository.saveAndFlush(entradaMiembros);
        Landing oficina = LandingResourceIT.createEntity(em);
        em.persist(oficina);
        em.flush();
        entradaMiembros.setOficina(oficina);
        entradaMiembrosRepository.saveAndFlush(entradaMiembros);
        Long oficinaId = oficina.getId();

        // Get all the entradaMiembrosList where oficina equals to oficinaId
        defaultEntradaMiembrosShouldBeFound("oficinaId.equals=" + oficinaId);

        // Get all the entradaMiembrosList where oficina equals to oficinaId + 1
        defaultEntradaMiembrosShouldNotBeFound("oficinaId.equals=" + (oficinaId + 1));
    }
