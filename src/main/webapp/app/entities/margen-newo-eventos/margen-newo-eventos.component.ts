...
export class MargenNewoEventosComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.margenNewoEventosService
        ...
        .subscribe(
          (res: HttpResponse<IMargenNewoEventos[]>) => this.paginateMargenNewoEventos(res.body, res.headers),
  ...
  registerChangeInMargenNewoEventos() {
    this.eventSubscriber = this.eventManager.subscribe('margenNewoEventosListModification', response => this.reset());
  }
  ...
  protected paginateMargenNewoEventos(data: IMargenNewoEventos[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.margenNewoEventos.push(data[i]);
    }
  }
