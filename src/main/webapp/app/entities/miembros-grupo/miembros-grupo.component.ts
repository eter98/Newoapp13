...
export class MiembrosGrupoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.miembrosGrupoService
        ...
        .subscribe(
          (res: HttpResponse<IMiembrosGrupo[]>) => this.paginateMiembrosGrupos(res.body, res.headers),
  ...
  registerChangeInMiembrosGrupos() {
    this.eventSubscriber = this.eventManager.subscribe('miembrosGrupoListModification', response => this.reset());
  }
  ...
  protected paginateMiembrosGrupos(data: IMiembrosGrupo[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.miembrosGrupos.push(data[i]);
    }
  }
