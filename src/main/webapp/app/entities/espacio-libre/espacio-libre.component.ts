...
export class EspacioLibreComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.espacioLibreService
        ...
        .subscribe(
          (res: HttpResponse<IEspacioLibre[]>) => this.paginateEspacioLibres(res.body, res.headers),
  ...
  registerChangeInEspacioLibres() {
    this.eventSubscriber = this.eventManager.subscribe('espacioLibreListModification', response => this.reset());
  }
  ...
  protected paginateEspacioLibres(data: IEspacioLibre[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.espacioLibres.push(data[i]);
    }
  }
