...
export class ConsumoMarketComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.consumoMarketService
        ...
        .subscribe(
          (res: HttpResponse<IConsumoMarket[]>) => this.paginateConsumoMarkets(res.body, res.headers),
  ...
  registerChangeInConsumoMarkets() {
    this.eventSubscriber = this.eventManager.subscribe('consumoMarketListModification', response => this.reset());
  }
  ...
  protected paginateConsumoMarkets(data: IConsumoMarket[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.consumoMarkets.push(data[i]);
    }
  }
