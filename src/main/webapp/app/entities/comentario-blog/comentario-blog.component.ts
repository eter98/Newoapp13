...
export class ComentarioBlogComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.comentarioBlogService
        ...
        .subscribe(
          (res: HttpResponse<IComentarioBlog[]>) => this.paginateComentarioBlogs(res.body, res.headers),
  ...
  trackId(index: number, item: IComentarioBlog) {
    return item.id;
  }
  ...
  registerChangeInComentarioBlogs() {
    this.eventSubscriber = this.eventManager.subscribe('comentarioBlogListModification', response => this.reset());
  }
  ...
  protected paginateComentarioBlogs(data: IComentarioBlog[], headers: HttpHeaders) {
