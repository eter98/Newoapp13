...
export class TipoRecursoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.tipoRecursoService
        ...
        .subscribe(
          (res: HttpResponse<ITipoRecurso[]>) => this.paginateTipoRecursos(res.body, res.headers),
  ...
  registerChangeInTipoRecursos() {
    this.eventSubscriber = this.eventManager.subscribe('tipoRecursoListModification', response => this.reset());
  }
  ...
  protected paginateTipoRecursos(data: ITipoRecurso[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.tipoRecursos.push(data[i]);
    }
  }
