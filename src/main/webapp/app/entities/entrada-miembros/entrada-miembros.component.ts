...
export class EntradaMiembrosComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.entradaMiembrosService
        ...
        .subscribe(
          (res: HttpResponse<IEntradaMiembros[]>) => this.paginateEntradaMiembros(res.body, res.headers),
  ...
  registerChangeInEntradaMiembros() {
    this.eventSubscriber = this.eventManager.subscribe('entradaMiembrosListModification', response => this.reset());
  }
  ...
  protected paginateEntradaMiembros(data: IEntradaMiembros[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.entradaMiembros.push(data[i]);
    }
  }
