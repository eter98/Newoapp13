...
export class UsoRecursoFisicoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.usoRecursoFisicoService
        ...
        .subscribe(
          (res: HttpResponse<IUsoRecursoFisico[]>) => this.paginateUsoRecursoFisicos(res.body, res.headers),
  ...
  registerChangeInUsoRecursoFisicos() {
    this.eventSubscriber = this.eventManager.subscribe('usoRecursoFisicoListModification', response => this.reset());
  }
  ...
  protected paginateUsoRecursoFisicos(data: IUsoRecursoFisico[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.usoRecursoFisicos.push(data[i]);
    }
  }
