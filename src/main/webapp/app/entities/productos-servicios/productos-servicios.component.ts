...
export class ProductosServiciosComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.productosServiciosService
        ...
        .subscribe(
          (res: HttpResponse<IProductosServicios[]>) => this.paginateProductosServicios(res.body, res.headers),
  ...
  registerChangeInProductosServicios() {
    this.eventSubscriber = this.eventManager.subscribe('productosServiciosListModification', response => this.reset());
  }
  ...
  protected paginateProductosServicios(data: IProductosServicios[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.productosServicios.push(data[i]);
    }
  }
