...
export class EquipoEmpresasComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.equipoEmpresasService
        ...
        .subscribe(
          (res: HttpResponse<IEquipoEmpresas[]>) => this.paginateEquipoEmpresas(res.body, res.headers),
  ...
  registerChangeInEquipoEmpresas() {
    this.eventSubscriber = this.eventManager.subscribe('equipoEmpresasListModification', response => this.reset());
  }
  ...
  protected paginateEquipoEmpresas(data: IEquipoEmpresas[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.equipoEmpresas.push(data[i]);
    }
  }
