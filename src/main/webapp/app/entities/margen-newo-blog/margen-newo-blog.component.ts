...
export class MargenNewoBlogComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.margenNewoBlogService
        ...
        .subscribe(
          (res: HttpResponse<IMargenNewoBlog[]>) => this.paginateMargenNewoBlogs(res.body, res.headers),
  ...
  registerChangeInMargenNewoBlogs() {
    this.eventSubscriber = this.eventManager.subscribe('margenNewoBlogListModification', response => this.reset());
  }
  ...
  protected paginateMargenNewoBlogs(data: IMargenNewoBlog[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.margenNewoBlogs.push(data[i]);
    }
  }
