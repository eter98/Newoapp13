...
export class TipoEspacioComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.tipoEspacioService
        ...
        .subscribe(
          (res: HttpResponse<ITipoEspacio[]>) => this.paginateTipoEspacios(res.body, res.headers),
  ...
  registerChangeInTipoEspacios() {
    this.eventSubscriber = this.eventManager.subscribe('tipoEspacioListModification', response => this.reset());
  }
  ...
  protected paginateTipoEspacios(data: ITipoEspacio[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.tipoEspacios.push(data[i]);
    }
  }
