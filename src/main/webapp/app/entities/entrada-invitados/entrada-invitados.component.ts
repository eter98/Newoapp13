...
export class EntradaInvitadosComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.entradaInvitadosService
        ...
        .subscribe(
          (res: HttpResponse<IEntradaInvitados[]>) => this.paginateEntradaInvitados(res.body, res.headers),
  ...
  registerChangeInEntradaInvitados() {
    this.eventSubscriber = this.eventManager.subscribe('entradaInvitadosListModification', response => this.reset());
  }
  ...
  protected paginateEntradaInvitados(data: IEntradaInvitados[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.entradaInvitados.push(data[i]);
    }
  }
