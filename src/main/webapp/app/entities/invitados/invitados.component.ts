...
export class InvitadosComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.invitadosService
        ...
        .subscribe(
          (res: HttpResponse<IInvitados[]>) => this.paginateInvitados(res.body, res.headers),
  ...
  registerChangeInInvitados() {
    this.eventSubscriber = this.eventManager.subscribe('invitadosListModification', response => this.reset());
  }
  ...
  protected paginateInvitados(data: IInvitados[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.invitados.push(data[i]);
    }
  }
